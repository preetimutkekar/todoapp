...
const Home = () => {
  ...
  useEffect(() => {
    ...
    socket.on("connect_error", (err) => {
      if (!connected) {
        fetchTodos();
        addToast(
          "Using standard mode - real-time updates unavailable",
          "warning"
        );
      }
    });
    ...
    socket.on("todo_deleted", (id, todoText, source) => {
      if (source !== clientId) {
        addToast(
          `Someone deleted a task${todoText ? `: "${todoText}"` : ""}`,
          "warning"
        );
      }
  ...
  const deleteTodo = async (id, text) => {
    try {
      await axios.delete(
        `http://localhost:5000/api/todos/${id}?clientId=${clientId}&text=${encodeURIComponent(
          text
        )}`
      );
  ...
  const filteredTodos = () => {
    let baseList = todos;
    if (filter === "active")
      baseList = baseList.filter((todo) => !todo.completed);
      ...
      <div className="max-w-2xl mx-auto mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 text-center">
            <h3 className="text-lg font-medium text-blue-500 dark:text-blue-300">
              All Tasks
            </h3>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 text-center">
            <h3 className="text-lg font-medium text-yellow-500 dark:text-yellow-300">
              Active
            </h3>
            <p className="text-2xl font-semibold mt-2 text-gray-800 dark:text-white">
              {stats.active}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 text-center">
            <h3 className="text-lg font-medium text-green-500 dark:text-green-300">
              Completed
            </h3>
            <p className="text-2xl font-semibold mt-2 text-gray-800 dark:text-white">
              {stats.completed}
        ...
        {filteredTodos().length === 0 && (
          <div className="text-center p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
            <p className="text-gray-600 dark:text-gray-400">
              No {filter !== "all" ? filter : ""} TODO found
            </p>
          </div>
        )}
      </div>
